...
const cartReducer = (
  ...
  switch (action.type) {
    ...
    case "SET_CART":
      ...
      case "DELETE_CART":
        return{...state, cart: {...state.cart, products: [...state.cart.products.filter(f=> f.productId !== action.payload)]}}
        case "DELETE_ONE_NUMBER":
        return{...state, 
          cart: {...state.cart, 
            products: [...state.cart.products.map(m => {
...
export const actions = {
  getProductsCart: (data: addProductToCartType) =>
    ...
    addOneCart: (id: number) =>
    ({ type: "ADD_ONE_CART", payload: id } as const),
...
export const setToCartOneNumber = (id: number, number: number): ThunkType => async (dispatch) => {
  dispatch(actions.setNumberCart(id, number));
};
export const addToCartOneNumber = 
(id: number): ThunkType => async (dispatch) => {
